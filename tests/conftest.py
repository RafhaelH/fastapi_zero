...
@contextmanager
def _mock_db_time(model, time=datetime(2025, 5, 20), updated_time=None):
    updated_time = updated_time or time

    def fake_insert_hook(_mapper, _connection, target):
        if hasattr(target, 'created_at'):
            target.created_at = time
        if hasattr(target, 'updated_at'):
            target.updated_at = time

    def fake_update_hook(_mapper, _connection, target):
        if hasattr(target, 'updated_at'):
